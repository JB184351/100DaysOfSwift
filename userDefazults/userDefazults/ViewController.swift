...
class ViewController: UITableViewController {
    ...
    @objc func startGame() {
        ...
        startWord = title!
        combinedWords.removeAll(keepingCapacity: true)
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isReal(word: lowerAnswer) {
                    combinedWords.insert(answer, at: 0)
                    saveWords(combinedwords: combinedWords)
    ...
    func isOriginal(word: String) -> Bool {
        return !combinedWords.contains(word)
    }
    ...
    func saveWords(combinedwords: [String]) {
